...
def convert_examples_to_features(text_a, text_b, tokenizer, max_seq_length):
    ...
    tokens_a = tokenizer.tokenize(text_a)  # 分字，可以处理中文和英文

    print("Tokenize: ",tokens_a)
    tokens_b = None
    if text_b:
    ...
    if tokens_b:
        ...
        _truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 3)  # 这个分支不走
    else:
    ...
    tokens.append("[CLS]")
    input_type_ids.append(0)  # segments
    ...
    input_ids = tokenizer.convert_tokens_to_ids(tokens)  # token 2 id
    print("Token for input: ",tokens)
    print("Token 2 id: ",input_ids)
    ...
    features = collections.OrderedDict()  # 有序的dict 
