...
class BertConfig:
    def __init__(self, bert_config_path, use_fp16, use_int8, use_strict, use_fc2_gemm, use_int8_skipln, use_int8_multihead, use_qat, use_sparsity, timing_cache):
        with open(bert_config_path, "r") as f:
            data = json.load(f)  # 加载bert_config.json
            self.num_attention_heads = data["num_attention_heads"]  # self attention中的头的个数 12
            self.hidden_size = data["hidden_size"]                  # hidden_size  768
            self.intermediate_size = data["intermediate_size"]
            self.num_hidden_layers = data["num_hidden_layers"]      #12
            self.head_size = self.hidden_size // self.num_attention_heads  # 每个头的大小
...
# 创建engine
def build_engine(batch_sizes, workspace_size, sequence_lengths, config, weights_dict, vocab_file):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(explicit_batch_flag) as network, builder.create_builder_config() as builder_config:
        ...
        #     if not config.use_qat:
                #   squad_json = 量化数据
                #   calibrationCacheFile = "bert_int8.cache"
                #   calib_num = 量化次数
...
def main():
    ...
    with build_engine(args.batch_size, args.workspace_size, args.sequence_length, config, weights_dict, args.vocab_file) as engine:
